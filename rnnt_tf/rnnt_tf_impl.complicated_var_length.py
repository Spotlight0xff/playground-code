...
# def select_diagonal(n=0, dim_limits=None):
#   """
...
#   :param dict[int -> tf.Tensor (B,)] dim_limits: specifies the valid lengths
#   :return:
#   """
#   assert isinstance(dim_limits, list)
#   assert len(dim_limits) == 2
#   idxs = tf.stack([tf.range(0, n), n - tf.range(n) - 1], axis=-1)
#
#   # idxs = axes_limit
#   # allow for more dims
#   n_time, n_target = dim_limits
#   idxs = py_print_iteration_info("idxs_pre(diag)", idxs, n, debug=debug)
#   idxs = tf.cond(n > n_time, lambda: idxs[:n_time], lambda: idxs)
#   idxs = tf.cond(n > n_target, lambda: idxs[n - n_target:], lambda: idxs)
#   idxs = py_print_iteration_info("idxs_post(diag)", idxs, n, debug=debug)
#   return idxs


def tf_forward_batched(log_probs_input, labels_input, input_lens, label_lens, blank_index, debug=False, sess=None):
  """
  ...
  :param input_lens: (B,) length of input frames
  :param label_lens: (B,) length of labels
  ...
  # mask out every invalid log-probs


  if debug:
    print("B=%d, T=%d, U=%d, V=%d" % (n_batch, max_time, max_target, n_vocab))
    input_lens = py_print_iteration_info("input_lens", input_lens, 0, debug=debug)
    label_lens = py_print_iteration_info("label_lens", label_lens, 0, debug=debug)
  ...
  def select_diagonal_batched(n, input_lens=None, label_lens=None, limit_start=0, limit_end=None, debug=False):
    """
    Helper function to index various diagonals in a 2D matrix, which can be non-square.
    One diagonal starts from the top-right and goes down to the bottom-left.
    `n=1` indices (with start_row=0, start_col=0):
    ...
    def build_mask(start, len, n_max, n_batch):
      """
      ...
      mask = tf.where(tf.logical_and(range_mat >= tf.expand_dims(start, axis=1),  # (B, 1)
                                     range_mat < tf.expand_dims(start + len, axis=1)),
    ...
    idxs_len_c = tf.where(cond_c,
                          input_lens + (label_lens+1) - n_tiled,  # phase (c)
    ...
    # combined mask for invalid indices
    mask = tf.logical_and(idxs_mask_b, idxs_mask_c)
    mask = py_print_iteration_info("indices mask", mask, n, debug=debug)

    # add constraints for limits
    # We want to be able to cut of the beginning (e.g. for blank-transition indexing)
    # or the end (e.g. for label transition)
    # We can't do normal slicing, because the diagonals may have different lengths in the batch
    tmp_indices = tf.where(mask)  # (K, 2)
    tmp_indices = py_print_iteration_info("tmp_indices", tmp_indices, n, debug=debug)
    segs_min = tf.cast(tf.segment_min(tmp_indices[:, 1], tmp_indices[:, 0]), tf.int32)  # (B,)
    segs_max = tf.cast(tf.segment_max(tmp_indices[:, 1], tmp_indices[:, 0]), tf.int32)  # (B,)

    segs_min = py_print_iteration_info("segs_min", segs_min, n, debug=debug)
    segs_max = py_print_iteration_info("segs_max", segs_max, n, debug=debug)
    limit_end = segs_max-1 if limit_end is None else limit_end
    len = (segs_max-segs_min+1)-limit_end
    len = py_print_iteration_info("indices mask_limits len", len, n, debug=debug)
    mask_limits = build_mask(start=segs_min+limit_start, len=len, #(segs_max-segs_min+1)-limit_end,
                             n_max=n, n_batch=n_batch)
    mask_limits = py_print_iteration_info("indices mask_limits", mask_limits, n, debug=debug)
    indices_masked = tf.boolean_mask(indices, mask_limits)
  ...
  def body_forward(n, alpha_ta):
    ...
    #idxs_blank = select_diagonal_batched(n + 1, input_lens, label_lens)  # (B*N', 3)
    #idxs_blank = tf.reshape(idxs_blank, (n_batch, -1, 3))
    #idxs_blank = idxs_blank[:, :-1, :]
    #idxs_blank = tf.reshape(idxs_blank, (-1, 3))

    idxs_blank = select_diagonal_batched(n + 1, input_lens, label_lens, limit_start=0, limit_end=1, debug=False)  # (B*N', 3)
    ...
    # We assume that log-props for invalid indices are set to NEG_INF
    # Otherwise we may have varying diagonal lengths (per example in the batch),
    # which complicates everything.
    lp_blank = tf.gather_nd(log_probs[:, :, :, blank_index], idxs_blank)  # (B*sum(N'))
    #lp_blank = tf.reshape(lp_blank, (n_batch, -1))  # (B, N)
    ...
    idxs_y = select_diagonal_batched(n + 1, input_lens, label_lens, limit_start=1, limit_end=None, debug=True)  # (B*N', 3)
    #idxs_y = tf.reshape(idxs_y, (n_batch, -1, 3))
    #idxs_y = idxs_y[:, 1:, :]
    #idxs_y = tf.reshape(idxs_y, (-1, 3))


    idxs_labels = tf.stack([idxs_y[..., 0], idxs_y[..., 2]], axis=-1)
    ...
    # we need the first few items, then concat -neg-inf to
    # not perform the addition for the last item (first column)
    lp_y = tf.gather_nd(log_probs, idxs_w_labels)  # (sum(N'),)
    ...
    # alpha_y: (B, N)
    # lp_y: (B, N)
    red_op = tf.stack([alpha_blank + lp_blank, alpha_y + lp_y], axis=0)  # (2, sum(N'))
    # NOTE: this does not include the precomputed values (first row/column)
    red_op = py_print_iteration_info("red-op", red_op, n, debug=debug)
    new_diagonal = tf.math.reduce_logsumexp(red_op, axis=0)  # (sum(N'),)
    ...
    new_diagonal = tf.cond(n < max_time-1,
                           ...
                           lambda: new_diagonal)
    #
    # a = tf.expand_dims(precomputed_row[:, n + 1], axis=1)  # (B, 1)
    # b = new_diagonal  # (B, N+1)
    # c = tf.expand_dims(precomputed_col[:, n + 1], axis=1)  # (B, 1)
    # new_diagonal = tf.concat([a, b, c], axis=1)
    new_diagonal = py_print_iteration_info("new_diagonal", new_diagonal, n, debug=debug)
    return [n + 1, alpha_ta.write(n, new_diagonal)]
  ...
  # (B,)
  a = alpha_out_ta.read(alpha_out_ta.size() - 1)[:, 0]
  indices = tf.stack([
    tf.range(n_batch),
    input_lens-1,
    label_lens,
...
def test_impl(name, acts, labels, blank_index, with_warprnnt=True):
  ...
  with sess.as_default():
    alpha_tf, ll_tf, grads_tf = tf_forward(log_probs, labels, blank_index=blank_index, debug=True, sess=sess)
...
def test_batched():
  ...
  # np.testing.assert_allclose(alpha_tf, alpha_ref)
  # print("TF vs ref: alpha matrices", colored("MATCH", "green"))
  np.testing.assert_almost_equal(costs_tf, -expected_costs, decimal=6)
  print("TF vs ref: log posterior", colored("MATCH", "green"))
  np.testing.assert_almost_equal(grads_tf, expected_grads, decimal=6)
  print("TF vs ref: gradients", colored("MATCH", "green"))
